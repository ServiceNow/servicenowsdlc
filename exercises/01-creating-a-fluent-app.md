...
Create a new Fluent application using nowSDK, and connect it to source control — the two pieces of tooling everything else today builds on. This is the same app you'll keep building on all day — Build Agent extends it in Exercise 03, and it's what gets pushed, released, and deployed in the exercises after that. By the end of this exercise, everyone should be logged in to their assigned sandbox — that's the baseline the rest of the day assumes.
...
- [ ] Find your name on the [instance sign-up sheet](https://servicenow-my.sharepoint.com/:x:/p/shelby_cohen/IQDPcQEo1cNpT7XNsBeEi9J_AYPIviiz8XC2P7KjYkhbBhg) — your sandbox was allocated ahead of time, and your row has its URL and your credentials (Setup Instructions steps 1-2 below cover this if you haven't yet)
...
We allocated everyone's sandbox ahead of today's session, so there's no self-service allocation step — just find yours and log in.

1. Find your name on the [instance sign-up sheet](https://servicenow-my.sharepoint.com/:x:/p/shelby_cohen/IQDPcQEo1cNpT7XNsBeEi9J_AYPIviiz8XC2P7KjYkhbBhg) — it lists your assigned sandbox and credentials.
2. Log in to your sandbox with those credentials — use the URL listed in the **Instance Link** column for your row on the sign-up sheet (it'll be under `empsdlcdev` or `empsdlcdev2`). That sandbox's URL is the `<instance url>` used for the rest of this exercise.

> Want to see how sandbox allocation actually works, or need to allocate one yourself later? See the [Appendix](#appendix-allocating-a-sandbox-yourself) at the end of this exercise.
...
6. Set up an authentication profile for your sandbox: `now-sdk auth --add <instance url> --type basic`
    - `<instance url>` is the sandbox URL from Setup Instructions step 2 above (e.g. `https://<sandboxalias>.<base-instance>.service-now.com`) — not the base instance URL.
...
5. Connect your sandbox to this repo:
    - Open the ServiceNow IDE on your sandbox instance (the `<instance url>` from Setup Instructions step 2). If nothing is open yet, the Explorer panel offers **Clone Git repository** alongside **Create an app** and **Open Apps** — click it.
...
## Appendix: Allocating a Sandbox Yourself

We allocated everyone's sandbox ahead of today's session, so you shouldn't need this — it's here for reference, or for when you allocate a sandbox on your own later.

1. Log in to the base instance (the URL listed in the **Instance Link** column for your row on the sign-up sheet — `empsdlcdev` or `empsdlcdev2`).
2. On your base instance, go to `<instance url>/now/developer-sandbox/home` directly, or search `sandbox` in the top nav and open **Sandbox Management Home**.

    ![Navigate to Sandbox Management Home](images/sandbox-setup/01-navigate-to-sandbox-management.png)
3. Click **Allocate sandbox**.

    ![Allocate a sandbox](images/sandbox-setup/02-allocate-sandbox.png)
4. In the **Allocate Sandbox** dialog, leave **Sandbox template** empty. Set **Sandbox alias** to the same `<your_name>` convention as the rest of this exercise (max 8 characters, must be unique), so it doesn't collide with anyone else's, then click **Allocate**.

    ![Allocate Sandbox dialog, with Sandbox template left empty](images/sandbox-setup/03-allocate-sandbox-details.png)
5. Wait for it to finish provisioning — that sandbox's URL is what you'd use as the `<instance url>` for the rest of this exercise.
